...
def print_topology_overview(npu_status):
    ...
    print()
    print(f" {cyan('Component 2: AMD XDNA 2 NPU (Optional Accelerator)')}")
    ...
    print(f"   • Proven Value:       1.8x faster first token (TTFT) + ~2W always-on intent routing")
    print(f"   • Note:               Does NOT improve sustained decode speed (embedded MTP wins)")
    print(f"   • Memory Footprint:   ~0.2 GiB for the 0.8B NPU drafter")
    print()
...
def find_llama_server():
    ...
    candidates = [
        ...
        Path.home() / "source" / "strix-halo-rocmfpx-hub" / "engine" / "bin" / "llama-server",
