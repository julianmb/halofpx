...
class LemonadeConfigTests(unittest.TestCase):
    def test_lemonade_config_paths(self):
...
class LemonadeRegistryTests(unittest.TestCase):
    def test_lemonade_registry_prefix_resolution(self):
    ...
    def test_recognized_model_names_resolution(self):
...
class LemonadeApiTests(unittest.TestCase):
    def setUp(self):
        self.client = TestClient(app)

    def test_api_v1_models_lemonade_format(self):
    ...
    def test_api_v1_load_lemonade_alias(self):
        ...
        with mock.patch.object(engine_mgr, "load_model", mock_load_model):
            # 1. Standard HaloFPX format
            resp1 = self.client.post("/api/v1/load", json={"model_id": "test-model-1"})
            assert resp1.status_code == 200
            assert called_with["model_id"] == "test-model-1"

            # 2. Lemonade format with "model"
            resp2 = self.client.post("/api/v1/load", json={"model": "test-model-2"})
            assert resp2.status_code == 200
            assert called_with["model_id"] == "test-model-2"

            # 3. Neither provided -> 400
            resp3 = self.client.post("/api/v1/load", json={})
            assert resp3.status_code == 400

    def test_api_v1_delete_routes(self):
        ...
        with mock.patch.object(model_mgr, "delete_model", mock_delete_model):
            # DELETE /api/v1/models/{model_id}
            resp1 = self.client.delete("/api/v1/models/test-model-del")
            assert resp1.status_code == 200
            assert ("test-model-del", None) in deleted_models

            # POST /api/v1/delete with model
            resp2 = self.client.post("/api/v1/delete", json={"model": "test-model-del-2", "variant": "ROCmFP4"})
            assert resp2.status_code == 200
            assert ("test-model-del-2", "ROCmFP4") in deleted_models

    def test_api_models_includes_human_readable_name(self):
...
class CliParityTests(unittest.TestCase):
    def test_cli_subcommands_parsed(self):
